...
namespace MWGui
{
        ...
        try
        {
            switch (id)
            {
                case GM_Name:
                    ...
                    // Reuse the last confirmed character name on the next new game.
                    // Keep an in-progress name when the player goes back during character creation.
    ...
    void CharacterCreation::onNameDialogDone(WindowBase* parWindow)
    {
        if (mNameDialog)
        {
            ...
            // Morrowind player names are limited to 31 characters.
